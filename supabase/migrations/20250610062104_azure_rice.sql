/*
  ...
  1. Schema Changes
    - Add `is_email_verified` column to users table (boolean, default false)
    - Add `email_verification_token` column to users table (text, nullable)
    - Add index for token lookups

  2. Security
    - Update RLS policies to handle verification status
    - Add functions for token generation and validation
*/
...
-- You may want to remove this line if you want to require verification for all users
